...
- All filesystem writes use locks and atomic replacement, reject symlink traversal and compare expected source hashes. Locks coordinate Pressroom processes; arbitrary external writers cannot be locked by the app. The deployment snapshot remains immutable even if another program subsequently changes source.
