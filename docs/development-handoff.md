...
## v0.0.1 version preparation — 2026-09-20

Reset app/CLI/Cargo lockfile, CMake and companion version labels to 0.0.1. Development releases iterate through 0.0.n; v0.1.0 is reserved for readiness. Release notes now describe the current editor, banner controls, providers and worker limitations. Earlier input manifests and verification entries remain historical. No local environment was available for this metadata change; use the PR CI result. The release workflow remains explicitly dispatched and produces a prerelease tarball and checksums after its validation gates. The older Arch recipe remains a separately pinned development snapshot and is not advertised as a v0.0.1 package. OPR submission remains after XPS acceptance.
