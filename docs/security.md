...
Pressroom is a single-user local application. It protects against unsafe Markdown, accidental path traversal, stale reviews, draft leakage and uncertain deployment outcomes. It does not isolate malicious software already running as the same operating-system user.
...
- Source reads and writes reject symlink traversal. Atomic writes and process locks coordinate Pressroom instances. External programs are outside those locks; source-hash guards detect changes before state-changing actions.
