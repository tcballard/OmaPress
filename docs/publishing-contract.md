...
A publication has a source folder, a canonical base URL, and an explicit GitHub repository/branch destination. `gh` owns authentication; Git invokes `gh auth git-credential`. Pressroom never reads a token into its JSON state.
...
Initialisation accepts a configurable URL. Before first publication, set the actual GitHub Pages URL or confirmed custom domain. Pages branch setup is created only if absent; an existing differing Pages source blocks setup instead of being overwritten. Configure custom-domain DNS and HTTPS with GitHub's normal ownership checks. Pressroom verifies the configured canonical site after deployment.
