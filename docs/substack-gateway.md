...
OmaPress can prepare and schedule articles through the self-hosted
...
- The gateway maps scheduling to Substack's `/drafts/{id}/scheduled_release`.
  ...
  readback of a later publication. OmaPress records **scheduled**, not published.
...
Substack accepts the schedule. A remote gateway must use HTTPS; OmaPress allows
...
OmaPress stores them in Secret Service; saving a configuration does not claim
...
`OMAPRESS_SUBSTACK_GATEWAY_CREDENTIALS_FILE` pointing to an absolute owner-only
...
`secret-tool clear application omapress service substack-gateway`; for an explicit
...
3. Open Substack, inspect the actual rendered draft and its social preview. Select
   ...
   OmaPress. Supported values are everyone or paid-only; email-off is not offered
   because that contract has not been verified.
4. Confirm the review and choose **Schedule on Substack**. OmaPress verifies the
...
After an uncertain mutating request, OmaPress stops; inspect Substack rather than
