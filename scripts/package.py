...
shutil.copytree(root/'deploy/worker',bundle/'share/omapress/worker')
shutil.copytree(root/'companion',bundle/'share/omapress/companion')
shutil.copy2(root/'scripts/omapress-companion',bundle/'bin/omapress-companion')
shutil.copy2(root/'scripts/install-companion.py',bundle/'share/omapress/install-companion.py')
...
(bundle/'build-info.json').write_text(json.dumps({'version':version,'commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),'executable_bytes':size,'shared_dependencies':['Qt6 Core/Gui/Qml/Quick/QuickControls2','git','gh','curl','secret-tool (libsecret)','xdg-open (xdg-utils)']},indent=2)+'\n')
